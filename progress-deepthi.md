...
### [2026-08-02 14:20 IST] — Fixed the Prava setup: three separate faults, one of them not what it looked like
- Prompt: the Prava payment step was failing, teammates on Linux could not fix it, credentials were said to be in `.env`; fix it without changing logic and do not push until it runs.
- Files changed: `package.json` (npm script flags only), `README.md`, `progress-deepthi.md`. **No source, no logic.** `.env` was created locally and is gitignored and untracked — verified before every commit.
- **Fault 1 — the file was named `.env.txt`, not `.env`.** Every script runs `node --env-file=.env`, so nothing was loaded at all and every value read as undefined. Almost certainly a Windows "Save as" appending `.txt`.
- **Fault 2 — five required variables were absent** even after renaming: `PRAVA_TEST_MANDATE_AMOUNT_CAP`, `PRAVA_TEST_MERCHANT_NAME`, `PRAVA_TEST_MERCHANT_URL`, `PRAVA_TEST_MERCHANT_COUNTRY`, `PRAVA_TEST_PRODUCT_DESCRIPTION`. Proved rather than assumed: `createPravaMandateSession.js` validates exactly those five, and loading the old file showed all five missing. Filled from `.env.example`.
- **Fault 3 — the one that was not what it looked like.** With credentials loading, calls still failed intermittently with `fetch failed` / `ETIMEDOUT`. The host resolves to two IPv4 addresses and two NAT64 IPv6 addresses; the IPv6 ones are unroutable here.
- **I got fault 3 wrong first and had to correct it.** I concluded it was DNS ordering and applied `--dns-result-order=ipv4first` on a 3-sample test that passed 3/3. A larger sample then failed 8/8 with the same flag, and checking Node's resolver showed it already returns IPv4 first by default — so the flag was treating a cause that did not exist. Recording this because a 3-run sample on an intermittent fault is not evidence, and I presented it as if it were.
- Real cause: `curl` succeeded 4/4 while Node failed 2/4 at the same moment, so the network was fine and Node's own connection logic was not. Node's Happy Eyeballs auto-selection abandons the attempt after roughly 250ms and surfaces `ETIMEDOUT`. Measured: **9/12** succeeded with no flag, **12/12** with `--network-family-autoselection-attempt-timeout=2000`. Then 6/6 clean through the npm script.
- Decision — **raise the auto-selection timeout rather than disable auto-selection.** `--no-network-family-autoselection` also gave 6/6, but it removes the IPv6 fallback entirely, which could break a teammate on a genuinely IPv6-only network. Raising the timeout keeps the fallback and only stops the premature abandonment.
- Decision — **the flag goes on every npm script that reaches Prava, including `start`.** A server that intermittently fails a charge mid-demo is the worst possible version of this bug.
- Also created one fresh mandate session; it returned a valid short-lived hosted approval URL. The passkey and card entry are human-presence controls on Prava's hosted page and were left to the user, as `precaution.md` requires.
- Validation: 78/78 tests; `prava:verify` 6/6 clean through the npm script; confirmed `.env` and `.env.txt` are both gitignored and untracked, and that only `package.json` and `README.md` were staged.
...
### [2026-08-02 IST] — Fixed the /deliberate flicker, added place suggestions, resolved a design conflict
- Prompt: the `/deliberate` page glitched; then add a suggestion dropdown to the location fields; then pull, push and merge.
- Files changed: `frontend/src/App.jsx`, `frontend/src/pages/Intake.jsx`, `frontend/src/lib/places.js` (new), `frontend/test/{autoAdvance,places}.test.js` (new), `frontend/test/render.ssr.jsx`, `progress-deepthi.md`.
- **The flicker was an infinite navigation loop, not a rendering problem.** `AutoAdvance`'s "send them back" branch did not test the same conditions that sent the user forward, so with an approval outstanding `/deliberate` sent you to `/approve`, which sent you straight back — several times a second. It used `pushState`, so every bounce also stuffed an entry into the back button.
- **A fixed-point test caught a second loop my first fix missed.** I wrote a test asserting that applying the routing rule to its own output must not move again. It immediately failed on a case I had not considered: with a choice *and* an approval both outstanding it ping-ponged `/approve ↔ /choose`. It also proved a receipt did not actually win — an outstanding approval would drag the user off `/receipt`. A render test could never have caught either; a loop is invisible to a single render.
- Decision — **rewrote the rule as stage precedence** ("if this stage is active, be on its page") instead of a chain of forward/backward rules. The chain form is what let two different pairs of branches send each other in circles. It also corrected an assumption I had written down wrongly: **choices outrank approval**, because you pick options first and then approve the plan containing them. Ranking approval higher was the direct cause of the second loop. All automatic moves now use `replace`.
- Place suggestions: the screenshot showed the goal preview reading "Plan a 3-day trip **from  to** for 1 traveler" — the city fields were empty and only showing placeholders, which is exactly the mistake a picker prevents.
- Decision — **bundled the airport list rather than calling a places API.** An API needs a key, a network round trip per keystroke, and a failure mode in the middle of a judged demo. For ~50 airports that is all cost, and bundling means the form works offline.
- Decision — **native `<datalist>`, not a custom combobox.** The browser provides correct keyboard handling, screen-reader semantics and mobile behaviour for free; a hand-rolled listbox is a well-known accessibility trap, and accessibility is my ownership. The cost is that the popup cannot be styled.
- Decision — **ties break on dataset order, not alphabetically.** My first version ranked Bali above Bengaluru for "B", inverting the India-first ordering the dataset already encodes. Caught by a test I had written expecting the opposite.
- **Merge conflict resolved as a design decision, not a text merge.** `origin/main` had replaced the intake form with an eight-step conversational wizard that deliberately removes airport codes entirely ("No airport codes. Humsafar will work out the route."). My work had added autocomplete to the old form's four fields, two of which no longer exist. I took main's design wholesale and re-applied suggestions only to the two place questions that remain, dropping the IATA autocomplete rather than fighting the newer, deliberate direction. The suggestions stay a shortcut and never a constraint, because that flow is free-first and "somewhere quiet near the sea" must remain a valid answer.
- Validation: 118/118 JS, Python OK, render smoke test, clean production build — all run after the conflict resolution, not before.
- Blocked on: nothing of mine. Remaining items are deploy, video and Devfolio, which need a human or external access.
- Needs from Jeswin/Preethesh: nothing. Flagging one product rough edge for whoever owns it: a run left sitting at a choice or approval holds the single active-run slot, so a judge who gets distracted and retries hits a 409 with no way out. A cancel control or auto-release on browser disconnect would remove that.
- Commit: `cfabf3f` (pushed to `deepthi/frontend-demo`, merged to `main`)

### [2026-08-02 IST] — Fixed hardcoded trip scope in backend discovery; added map links
- Prompt: asked whether the traveller stays in one hotel and eats at one place, then to fix the hardcoded values and integrate Google Maps.
- Files changed: `backend/src/services/tripScope.js` (new), `backend/test/tripScope.test.js` (new), `backend/src/services/discoveryService.js`, `backend/src/fixtures/discovery.js`, `frontend/src/lib/maps.js` (new), `frontend/test/maps.test.js` (new), `frontend/src/pages/Choose.jsx`, `frontend/src/App.jsx`, `frontend/src/styles.css`, `frontend/test/render.ssr.jsx`.
- **Reproduced before fixing.** The agent-side fixtures already scaled by days/travellers/rooms, but the **backend discovery route did not** — and that route is what a `--live-discovery` run actually uses, so the two paths disagreed. `POST /api/discovery/stay` returned "2 nights, pool-view room" at ₹11,200 for both a 2-day solo trip and an 8-day trip for four.
- Decision — **scale on the way out of the service, never in the fixture rows.** `integrations.test.js` locks the raw totals to ₹16,100 / ₹35,600 to guarantee the agents have real contention to negotiate. Editing prices would have silently broken that. A test now asserts the baseline trip (3 days, 1 traveller, 1 room) is a **mathematical no-op**, so the invariant cannot quietly stop meaning anything.
- Decision — **per-category multipliers**, because these do not grow alike: a hotel with nights × rooms, meals with days × travellers, a seat or activity ticket with travellers only. A 30-day trip must not make a flight more expensive. Pinned by tests.
- Decision — **live provider responses are never scaled.** Duffel already prices the real request; scaling again would double-count.
- Decision — **stripped the baked-in scope from fixture descriptions and generate it.** Otherwise a row read "Dinner for two plus one lunch · 8 days of meals for 4", contradicting itself. This is the `precaution.md` problem rather than an arithmetic one: the UI was describing a trip the plan did not deliver. Verified nothing asserts descriptions before editing them.
- Also pinned that **cheaper options stay cheaper at every trip size** — if scaling reordered the list, the negotiation would stop meaning anything.
- Maps: every option now links to the real place. Decision — **a keyless `?api=1` search URL, not an embedded map.** The Maps JS/Embed APIs need a browser-exposed key, and a key in client JavaScript is public to every visitor; an embed is also another thing that can fail mid-demo. A test asserts no key can reach client markup.
- Decision — **the map link sits outside the choose button.** An `<a>` nested in a `<button>` is invalid HTML, traps the keyboard, and would have meant every attempt to *look at* a place *selected* it. A regex assertion enforces no anchor is ever nested in a button.
- **Answered but deliberately not fixed:** one agent buys exactly one option, so the traveller stays in one hotel and eats at one vendor. That is not a fixture bug — it falls out of the core mechanic, since one agent maps to one merchant-scoped credential and a `listed` mandate is locked to one merchant. Multi-vendor food would need several mandates and several human passkey ceremonies per agent. Flagged as the strongest "what happens next" answer rather than re-architecting Jeswin's orchestrator and Preethesh's payment layer on deadline day without asking.
- Validation: 139/139 tests, clean build, and the fix verified against the live route — 2 days → "1 night, 1 room" ₹5,600; 3 days → unchanged ₹11,200; 8 days/4 people/2 rooms → "7 nights, 2 rooms" ₹78,400.
- Note: this touches Preethesh's fixtures and discovery service. Prices, ratings, vendors, ids, ordering and `source` are all preserved exactly; only "how much, and for what" changed.
